...
- **Mechanik (korrigiert nach Architect-Review MAJOR-1 — `--gateway`/`--token`-Flags existieren NICHT, verifiziert CLI-Help v2026.7.1-2):**
  ...
  - Status-/Session-Abfragen: `openclaw gateway call --url <url> --token <token> …`
...
1. **Tokens:** Lieferst du 3 bestehende Gateway-Tokens (je Instanz) oder erzeuge ich je Instanz einen Nova-dedizierten Token (Anleitung: `openclaw config set gateway.auth.token` — V2-Hinweis beachten: ENV gewinnt aktuell, d.h. Token-Wechsel braucht Deploy/Compose-ENV-Update oder Issue-#68-V2-Fix zuerst)?

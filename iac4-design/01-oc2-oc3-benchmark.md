...
- **Status:** Vorgeschlagen (Review Runde 1 eingearbeitet, Re-Review ausstehend)
- **Datum:** 2026-08-01
...
- **B1 – Modell-Invertierung:** `openclaw_agent_models` setzt Orchestrator/Engineer auf `v4-flash` (billig), Architect/Reviewer auf `v4-pro` (teuer). Die offizielle Doku empfiehlt das Gegenteil: *„keep your main agent on a higher-quality model and keep your sub-agents on a cheaper model"* (`agents.defaults.subagents.model`). Quelle: [docs.openclaw.ai/tools/subagents](https://docs.openclaw.ai/tools/subagents). ✅ verifiziert (all.yml + subagents.md).
...
**Offizielle Doku (verifiziert lokal 2026-08-01, `/usr/lib/node_modules/openclaw/docs/`):**
- [tools/subagents.md](https://docs.openclaw.ai/tools/subagents) — Sub-Agent-Modelle („cheaper model for sub-agents"), delegationMode (suggest/prefer), maxSpawnDepth 2 = Orchestrator-Pattern, maxChildrenPerAgent/maxConcurrent (Default 8), runTimeoutSeconds (Default 0), announce chain, isolated/fork, tools.subagents-Policy (deny gateway/cron), allowAgents (Default: nur Requester)
